...
case 'lofi':
          ...
          case 'siri':
					hobby = body.slice(1)
					const hob =['Oi rsrs','Sim, Eu acho','Não sei dizer','Por quê?','Sei lá','Eu não quero falar sobre isso','Foda-se','Voce é uma gostosa','kkkkkk','Não fala comigo','Pesquisa no google','Lixo','Meu pau','Eu acho que voce tentou ser engraçado(a)','Eu quero comer uma bolacha','Fdp','Sopa de mamacor','Vou gozar flores']
					const by = hob[Math.floor(Math.random() * hob.length)]
					client.sendMessage(from, 'Pergunta : *'+hobby+'*\n\nResposta : '+ by, text, { quoted: mek })
					break
					case 'gay':
					hobby = body.slice(1)
					const hob =['3%','83%','30%','89%','20%','75%','99%','40%','12%','97%','99%','50%','100%','66%','70%','88%']
					const by = hob[Math.floor(Math.random() * hob.length)]
					client.sendMessage(from, '*「 ✔️ 」 Calculando...* : \n\nPorcentagem de gay : '+ by, text, { quoted: mek })
					break
                case 'bug':
                case 'reportar':
                case 'reportarbug':
